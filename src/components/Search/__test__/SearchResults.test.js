...
describe('SearchResults', () => {
    ...
    it('should render correctly with search results', () => {
        ...
        results.forEach(result => {
            ...
            expect(screen.getByText(`${result.views} views • a few seconds ago`)).toBeInTheDocument();
            expect(screen.getByText(result.description)).toBeInTheDocument();
            expect(screen.getByAltText(result.title)).toHaveAttribute('src', `https://apps.rubaktechie.me/uploads/thumbnails/${result.thumbnailUrl}`);
        });
    });
    ...
    it('should handle missing or incomplete data gracefully', () => {
        ...
        expect(screen.getByAltText('No title')).toHaveAttribute('src', 'https://apps.rubaktechie.me/uploads/thumbnails/null');
    ...
    it('should handle partially complete data', () => {
        ...
        expect(screen.getByAltText('Video 1')).toHaveAttribute('src', 'https://apps.rubaktechie.me/uploads/thumbnails/thumbnail1.jpg');
        expect(screen.getByText('Video 1')).toBeInTheDocument();
        expect(screen.getByText('No channel name')).toBeInTheDocument();
        expect(screen.getByText('100 views • a few seconds ago')).toBeInTheDocument();
