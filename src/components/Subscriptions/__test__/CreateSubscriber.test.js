...
describe("CreateSubscriber", () => {
  ...
  test("shows signInPopup when not authenticated and button is clicked", () => {
    ...
    expect(
      screen.getByText(/Sign in to subscribe to this channel./i)
    ).toBeInTheDocument();
    expect(
  ...
  test("toggles subscription when authenticated", async () => {
    ...
    const updatedSubscribeBtn = screen.getByRole("button", {
      name: /subscribed/i,
    });
  ...
  test("handles API errors on subscription failure", async () => {
    useAuth.mockReturnValue({ isAuthenticated: true });
    CreateSubscriberApi.mockResolvedValue({
      success: false,
  ...
  test("shows error message when subscription fails", async () => {
    useAuth.mockReturnValue({ isAuthenticated: true });
    CreateSubscriberApi.mockRejectedValue({
      success: false,
    ...
    expect(
      await screen.findByText("Error processing subscription")
    ).toBeInTheDocument();
  });
});
