...
describe('Feelings', () => {
    beforeEach(() => {
      ...
      test('renders initial likes and dislikes', () => {
        render(
            <Feelings videoId="123" initialLikes={10} initialDislikes={5} initialUserFeeling={null} />
        );
    ...
    test('handles like click with successful API response', async () => {
         ...
         await waitFor(() => {
             expect(updateFeelings).toHaveBeenCalledWith({ videoId: '123', type: 'like' });
         });
         await waitFor(() => {
    ...
    test('handles like click when userFeeling is dislike', async () => {
        ...
        render(
            <BrowserRouter>
                <Feelings videoId="123" initialLikes={10} initialDislikes={5} initialUserFeeling="dislike" />
            </BrowserRouter>
        );
    ...
    test('does not update likes when userFeeling is already like', async () => {
        render(
            <BrowserRouter>
                <Feelings videoId="123" initialLikes={10} initialDislikes={5} initialUserFeeling="like" />
            </BrowserRouter>
        );
    ...
    test('handles like click with failed API response', async () => {
        ...
        updateFeelings.mockResolvedValue({ success: false, error: 'Failed to update feelings' });
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to update feelings:', { success: false, error: 'Failed to update feelings' });
    ...
    test('handles dislike click', async () => {
        ...
        await waitFor(() => {
            expect(mockUpdateFeelings).toHaveBeenCalledWith({ videoId: '123', type: 'dislike' });
        });
        await waitFor(() => {
    ...
    test('handles dislike click when userFeeling is like', async () => {
        ...
        await waitFor(() => {
            expect(updateFeelings).toHaveBeenCalledWith({ videoId: '123', type: 'dislike' });
        });
    ...
    test('handles like click with thrown error', async () => {
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith('Error updating feelings:', mockError);
...
describe('updateFeelings function', () => {
    ...
    test('should update feelings successfully', async () => {
        ...
        const expectedResponse = { success: true, message: 'Feelings updated successfully' };
    ...
    test.skip('should handle successful feelings update', async () => {
        ...
        expect(response).toEqual(expectedResponse);
        expect(fetch).toHaveBeenCalledWith('https://apps.rubaktechie.me/api/v1/feelings/', {
            method: 'POST',
            headers: {
