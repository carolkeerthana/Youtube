...
describe('History', () => {
    ...
    test('clears all watch history when CLEAR ALL WATCH HISTORY button is clicked', async () => {
        ...
        await waitFor(() => expect(screen.getByTestId('notify-delete')).toHaveTextContent('Watch Histories Deleted Successfully'));
        await waitFor(() => {
            expect(screen.queryByText('Watch Histories Deleted Successfully')).not.toBeInTheDocument(); // Ensure notification disappears after 3 seconds
        }, { timeout: 3500 }); 
    });
    ...
    test('clears all search history when CLEAR ALL SEARCH HISTORY button is clicked', async () => {
        ...
        await waitFor(() => expect(deleteAllHistory).toHaveBeenCalledWith('search'));
        await waitFor(() => expect(screen.getByTestId('notify-delete')).toHaveTextContent('Search Histories Deleted Successfully'));
        await waitFor(() => {
            expect(screen.queryByText('Search Histories Deleted Successfully')).not.toBeInTheDocument(); // Ensure notification disappears after 3 seconds
        }, { timeout: 3500 });  
    });
    ...
    test('logs error to console when deleteAllHistory throws an error', async () => {
        ...
        await waitFor(() => expect(deleteAllHistory).toHaveBeenCalledWith('watch'));
        await waitFor(() => expect(consoleErrorSpy).toHaveBeenCalledWith('Error deleting histories:', expect.any(Error)));
