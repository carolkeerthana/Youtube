...
describe('WatchHistory', () => {
    ...
    beforeEach(() => {
        fetchHistories.mockResolvedValue({ success: true, data: mockHistory, totalPages: 1 }); // Mock the fetchHistories function
    });
    ...
    test('handles delete history', async () => {
        const setHistoryMock = jest.fn();
        render(
        <WatchHistory history={mockHistory} setHistory={setHistoryMock} setError={jest.fn()} />
        )
